...
void main() {
  ...
  testWidgets('Search query does not clear when state updates but query is not null', (tester) async {
    // Initial state with a search query
    when(() => mockBloc.state).thenReturn(const ProductListState(searchQuery: 'test'));

    // Create the stream controller to simulate state changes
    final streamController = StreamController<ProductListState>();
    whenListen(mockBloc, streamController.stream, initialState: const ProductListState(searchQuery: 'test'));
    ...
    streamController.add(const ProductListState(isLoading: true, searchQuery: 't'));
    await tester.pump();
  ...
  testWidgets('Search query clears only when state explicitly clears query', (tester) async {
  ...
  testWidgets('Screen wraps content in GestureDetector for keyboard dismissal', (tester) async {
    when(() => mockBloc.state).thenReturn(const ProductListState());
